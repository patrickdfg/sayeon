...
페이지가 상단 탭(성령사연 / 월명동사연 / 말씀)으로 이어져 있다.
...
설정 맨 위 **새 사연 알림 → 받기**를 누른 기기로, 새 성령사연(2026)이 올라오면
"새 성령 사연 — 성령 사연 168 이(가) 올라왔습니다" 알림이 가고, 누르면 그 편이 열린다.
안드로이드 크롬·삼성 인터넷은 된다. 아이폰은 홈 화면에 추가한 앱에서만,
카카오톡 안 브라우저는 안 된다.

| 파일 | 하는 일 |
| --- | --- |
| `settings.js` | '받기' 단추. 기기를 Supabase 명단에 올리고 내린다. 공개 열쇠 `VAPID_PUBLIC` 이 있다 |
| `sw.js` | 휴대폰에 온 알림을 띄우고, 누르면 `#n=편번호` 로 연다. 파일을 저장해 두지 않는다. **말씀 알림도 이 워커가 띄운다** |
| `tools/send_push.py` | 마지막 편을 명단에 보낸다. `--test` 는 시험 알림 |
| `supabase-push.sql` | 명단 표와 함수. Supabase SQL Editor 에서 한 번 실행해 두었다 |
| `work-upload.yml` 마지막 단계 | 자동 게시가 끝나면 보낸다 |
| `push-notify.yml` | 컴퓨터에서 `sayeon.json.enc` 를 직접 올렸을 때 보낸다. 손으로 돌리면 시험 알림 |

- **말씀(`malsseum`)도 이 명단을 같이 쓴다.** 같은 호스트라 같은 출처이고, 구독은
  `/sayeon/sw.js` 하나에 매여 있다. 말씀 저장소에는 `sw.js` 가 없고 보내는 쪽
  (`tools/send_push.py`·`push-notify.yml`)만 따로 두었다. 그래서 **알림 모양이나 누를 때
  동작을 고치면 말씀 알림도 같이 바뀐다.** `push_sent` 에서 말씀은 `mal-` 을 붙여 가른다.
  '받기'는 하나뿐이라 사연만 끄고 말씀만 받는 것은 안 된다.
- **같은 편은 한 번만 보낸다.** 보내기 전에 Supabase `push_sent` 에 편 번호를 먼저 적고,
  이미 있으면 건너뛴다. 그래서 두 일꾼이 겹쳐 돌거나 같은 편을 고쳐 올려도 또 가지 않는다.
- **사이트에 새 원고가 올라간 것을 확인하고 보낸다**(최대 10분 기다림). 알림을 눌렀는데
  옛 목록이 보이면 안 되기 때문이다.
- 저장소 비밀값: `VAPID_PRIVATE_KEY`(알림 서명), `PUSH_TOKEN`(명단 읽기).
  **없으면 알림만 건너뛰고 게시는 그대로 된다.**
  토큰을 바꾸면 `supabase-push.sql` 의 `push_admin` 해시도 바꿔 다시 실행한다.
- **처음 들어온 기기에는 화면 아래에 한 번 묻는다**("새 사연이 올라오면 알림을 받으시겠어요? [나중에] [받기]").
  성령사연·월명동이 암호를 풀고 들어간 뒤 `SaSettings.askPush()` 를 부른다.
  답하면 `localStorage.sayeonPushAsked` 에 적어 다시 묻지 않는다. 알림이 안 되는 브라우저,
  이미 받거나 막아 둔 기기에는 뜨지 않는다. 브라우저가 몰래 켜는 것은 막아 두어 '기본 받기'는 불가능하다.
- 알림 내용에는 **편 제목만** 넣는다. 원고를 잠가 둔 뜻을 살려 본문은 넣지 않는다.
- 공개 열쇠를 바꾸면 이미 받기를 누른 기기는 모두 다시 눌러야 한다. 되도록 바꾸지 않는다.
...
- 나중에 육성 녹음이 오면 **같은 번호로 파일만 바꿔 끼우고** sync 를 다시 만든다.
  손으로 할 일은 네 가지다.
  1. `audio/<번호>.m4a` 를 넣고 **컴퓨터 목소리 `.mp3` 는 지운다**
  2. 그 편의 `audio` 를 새 파일 이름으로 고친다 (확장자가 바뀐다)
  3. **`"tts": true` 를 지운다.** 안 지우면 뷰어가 계속 🤖 로 보여 준다
  4. `audio/sync.json` 에서 그 편 항목을 지우고 다시 만든다 —
     Actions 의 "성령사연 문단 시간표 만들기"(`build-sync.yml`) 를 돌리면 된다
- **말씀은 이 저장소의 `build_sync.py` 로 만들지 않는다.** 저장소가 갈라졌으므로
  `malsseum` 쪽 Actions 의 "말씀 문단 시간표 만들기" 를 쓴다.
...
- **localStorage 키는 페이지마다 접두어가 다르다.** 같은 도메인이라 저장소를
  ...
  **기기 목소리 고르기는 설정의 '읽어 주는 목소리' 칸에 있다**(2026-09-20 에 되살렸다).
  음성 파일이 있는 편은 그 파일을 틀고, 없는 편만 기기가 읽는다
  — 지금은 2025 성령사연 225편과 월명동 52편이 그렇다.

  - **생김새는 막대 하나**다. 고른 것만 보이다가 누르면 목록이 펼쳐진다(`.sa-bar`/`.sa-list`).
    목소리가 여덟 개씩 되면 낱개 단추로 늘어놓을 때 설정 화면을 다 먹기 때문이다.
    목록은 320px 까지만 보이고 그 뒤는 스크롤한다. 고르면 저절로 접힌다.
  - 목록은 `speechSynthesis.getVoices()` 가 내주는 한국어 목소리 **그대로**다.
    몇 개가 뜨는지는 기기마다 다르고 우리가 늘릴 수 없다. 앱(문피아 같은)은 시스템
    TTS 를 직접 불러서 더 많이 보이기도 한다.
  - 같은 이름이 두 번 나오는 기기가 많다. 기기 내장용과 네트워크용이 갈라져 있어서다.
    `localService === false` 면 뒤에 '(네트워크)' 를 붙이고, 그래도 겹치면 번호를 붙인다.
  - **`settings.js` 를 고쳤으면 세 페이지의 `settings.js?v=숫자` 를 같이 올린다.**
    안 올리면 폰에 옛 파일이 남아 고친 것이 안 보인다(막대로 바꿨는데 낱개 단추가
    그대로 보이는 일이 있었다). 자료 파일과 달리 스크립트는 시각을 못 붙인다
    — 매번 새로 받으면 느려지기 때문이다.
  - **고르는 일은 `settings.js` 의 `pickVoice()` 한 곳에서만 한다.**
    세 페이지는 `SaSettings.pickVoice()` 를 부르기만 한다. 페이지마다 따로 두지 말 것.
  - 목록은 브라우저가 늦게 채우기도 해서 `voiceschanged` 때 다시 그린다.
  - 고른 값(`voice`)은 voiceURI 다. **다른 기기에는 없는 이름일 수 있으므로**
    없으면 말없이 '기기가 고름' 으로 되돌아간다(`siteSettings` 를 두 대가 같이 쓴다).
